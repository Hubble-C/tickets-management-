...
public class LoginService : ILogin
{
    ...
    public LoginService(IDbConnectionFactory dbConnectionFactory,  IConfiguration configuration, IPasswordHasher<AspNetUsers> passwordHasher,  LoginValidator loginValidator)
    {
    ...
    public async Task<ServiceResponse<AspNetUsers>> Login(string username, string password)
    {
        ...
        using (var connection = _dbConnectionFactory.GetAuthConnection())
        {
            var userExist =
            await connection.QueryFirstOrDefaultAsync<AspNetUsers>(
